...
export function loadRecommendations() {
  const path = join(VSCODE_TEMPLATES, 'extensions.json');
  if (!existsSync(path)) return [];
  try {
    return JSON.parse(readFileSync(path, 'utf8')).recommendations ?? [];
  } catch (err) {
    log.warn(`Could not parse vscode/extensions.json: ${err.message}`);
    return [];
  }
}
