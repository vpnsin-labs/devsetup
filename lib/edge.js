...
function applyPoliciesWindows(policies, { dryRun }) {
  ...
  // Write the .reg into a freshly-created private temp dir (unpredictable name,
  // owner-only perms) so the file we `reg import` can't be pre-planted or swapped.
  const regDir = mkdtempSync(join(tmpdir(), 'devsetup-edge-'));
  const regPath = join(regDir, 'policies.reg');
...
function applyBookmarks(platform, { dryRun }) {
  ...
  mkdirSync(dirname(dest), { recursive: true }); // no-op if it already exists
  // Back up any existing file without a check-then-act race: attempt the rename
  // and tolerate "not found" rather than testing existence first.
  try {
