...
export function buildInstallCmd(spec) {
  ...
  // Keep winget fully non-interactive. --accept-*-agreements pre-accept the
  // license/source agreements, and --source winget confines resolution to the
  // community `winget` source (where all our IDs live) so it never touches the
  // `msstore` source, whose first-run "send your region / agree to terms" prompt
  // otherwise hangs an unattended install with no way to answer (our readline
  // owns stdin). All flags predate winget 1.0, so there's no version risk.
  if (spec.winget)
    return `winget install --silent --accept-source-agreements --accept-package-agreements --source winget --id ${spec.winget}`;
