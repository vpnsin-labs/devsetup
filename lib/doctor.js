  ...
  ollama: {
    ...
    run: () => {
      if (!hasCommand('ollama')) {
        return { status: 'warn', detail: 'not found (optional)', fix: 'devsetup --bootcamp' };
      }
      const v = parseVersion(capture('ollama', ['--version']));
      const models = capture('ollama', ['list']);
      // `ollama list` prints a header row even with zero models pulled.
      const hasModel = models ? models.split('\n').filter(Boolean).length > 1 : false;
      if (!hasModel) {
        return {
          status: 'warn',
          detail: `${v ? v.raw : 'installed'} — no model pulled yet`,
          fix: 'ollama pull llama3   (~4.7GB — do this on home wifi)',
        };
      }
      return { status: 'pass', detail: v ? v.raw : 'installed' };
    },
